...
from core.extraccion.constants.extractor import (
    DEFAULT_EXTRACT_CONFIG, CLEANTEXT_CONFIG,
    ACADEMIC_TERMS, SUBJECT_CODE_PATTERNS,
    TIME_BASE, TIME_PATTERNS,
    DAY_PATTERNS, CLASSROOM_PATTERNS,
...
class PDFExtractor:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.config = DEFAULT_EXTRACT_CONFIG.copy()
        if config:
    ...
    def _get_academic_patterns(self, text: str) -> Dict[str, Any]:
        ...
        """
        # 1. Detección mejorada de códigos de asignatura españoles
        ...
        # Patrones de horario
        time_patterns = re.findall(TIME_BASE, text)
        for pat in TIME_PATTERNS:
            time_patterns += re.findall(pat, text)

        # Días de la semana
        weekdays = []
        for pat in DAY_PATTERNS:
            weekdays += re.findall(pat, text, flags=re.IGNORECASE)

        # Patrones de aula
        classroom_patterns = []
        for pat in CLASSROOM_PATTERNS:
            classroom_patterns += re.findall(pat, text)
    ...
    def _calculate_academic_score(self, academic_metrics: Dict[str, Any]) -> float:
        ...
        # Componente de estructura de horarios
        schedule_component = (
        ...
        return codes_component + terminology_component + schedule_component
...
def get_pdf_extractor() -> PDFExtractor:
    """
    Factory function para obtener instancia global del extractor PDF nativo.
    
    Returns:
        PDFExtractor: Instancia configurada para extracción de PDFs académicos nativos
    """
